...
To evaluate the full anomaly detection pipeline using standard anomaly detection metrics, we employ a synthetic anomaly injection strategy. Specifically, 5\% of nodes in each dataset are randomly selected, and their features are perturbed through random permutation and Gaussian noise addition. This creates ground-truth anomaly labels, enabling the computation of AUC-ROC, F1-score, Precision, and Recall.

Since AnomEn operates at the \textit{subgraph level}, all nodes within an anomalous subgraph inherit the subgraph's anomaly label. This design choice reflects the method's focus on collective anomalies rather than individual outliers.
...
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Dataset} & \textbf{AUC} & \textbf{F1} & \textbf{Precision} & \textbf{Recall} \\
\hline
Cora & 0.4824 & 0.0948 & 0.0501 & 0.8815 \\
\hline
CiteSeer & 0.5054 & 0.0892 & 0.0472 & 0.7952 \\
\hline
PubMed & 0.5570 & 0.1196 & 0.0719 & 0.3543 \\
\hline
\end{tabular}%
}
\end{table}

As shown in Table~\ref{tab:anomaly_metrics}, AnomEn achieves high recall on Cora (0.88) and CiteSeer (0.80), indicating that the framework successfully captures most anomalous nodes within anomalous subgraphs. The lower precision values are expected because AnomEn flags entire subgraphs rather than individual nodes: when a subgraph is marked as anomalous, all its constituent nodes (including normal ones) are classified as anomalous. This trade-off is inherent to subgraph-level detection and represents a deliberate design choice that prioritizes identifying anomalous communities over pinpointing individual anomalous nodes.

\subsection{Ablation Study: Combined Score Threshold}

To empirically justify the combined anomaly score threshold ($> 0$), we evaluate the detection performance at different threshold values. Table~\ref{tab:ablation_score} presents the results on the Cora dataset.

\begin{table}[ht]
\centering
\caption{Effect of Combined Score Threshold on Anomaly Detection (Cora)}
\label{tab:ablation_score}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{|c|c|c|c|c|}
\hline
\textbf{Threshold} & \textbf{AUC} & \textbf{F1} & \textbf{Recall} & \textbf{Accuracy} \\
\hline
$> 0.0$ & 0.5366 & 0.1012 & \textbf{0.9037} & 0.2001 \\
\hline
$> 0.2$ & 0.5366 & 0.1012 & 0.9037 & 0.2001 \\
\hline
$> 0.4$ & 0.5366 & 0.1001 & 0.8222 & 0.2633 \\
\hline
$> 0.6$ & 0.5366 & 0.0979 & 0.4519 & 0.5849 \\
\hline
$> 0.8$ & 0.5366 & 0.0979 & 0.4519 & 0.5849 \\
\hline
$> 1.0$ & 0.5366 & 0.0000 & 0.0000 & 0.9501 \\
\hline
\end{tabular}%
}
\end{table}

The results reveal that a threshold of $> 0$ maximizes recall (0.90), which aligns with the primary objective of anomaly detection: to capture as many anomalous patterns as possible. As the threshold increases, recall drops sharply (e.g., at $> 0.6$, recall drops to 0.45). At threshold $> 1.0$, no anomalies are detected. The choice of $> 0$ ensures maximum sensitivity to anomalous subgraphs, which is preferred in security-critical applications where missing an anomaly is more costly than investigating false positives.

\subsection{Domain Context Interpretation of Anomalies}

In the context of citation networks such as Cora, CiteSeer, and PubMed, the anomalous subgraphs detected by AnomEn can be interpreted as groups of publications exhibiting unusual patterns:

\begin{itemize}
    \item \textbf{Sparse inter-cluster subgraphs} (low density): These may represent papers from different research areas that are loosely connected, potentially indicating interdisciplinary or cross-domain research clusters that deviate from the typical tightly-knit community structure.
    
    \item \textbf{Small isolated subgraphs} (low node count): These could correspond to niche research topics, emerging fields with few publications, or orphaned papers with limited citation connections.
    
    \item \textbf{Highly dispersed subgraphs} (high average distance): These may indicate papers whose feature representations (topics, keywords) are highly diverse despite being clustered together, suggesting potential misclassification or papers spanning multiple research domains.
\end{itemize}

For example, on the Cora dataset (7 classes representing research topics), 5 out of 7 subgraphs were flagged as anomalous. The 2 normal subgraphs correspond to well-defined, cohesive research communities, while the 5 anomalous subgraphs exhibit characteristics such as unusual density or size that warrant further investigation by domain experts.

This interpretability is a key advantage of AnomEn over black-box methods like DOMINANT and AnomalyDAE, which provide reconstruction error scores without explaining \textit{why} a node or subgraph is anomalous.

\section{CONCLUSION}

This paper presents AnomEn, an unsupervised graph neural network-based framework for subgraph anomaly detection. The key contributions of this work are:

\begin{enumerate}
    \item A GNN-based encoder that balances self-features and neighbor features through a weighted parameter $\beta$, mitigating the influence of anomalous neighbors during message passing.
    
    \item The CISS (Centroid Initialization by Selective Sampling) algorithm for K-means centroid initialization, which systematically distributes initial centroids across the data space, achieving improvements of up to 14.06\% in Davies--Bouldin Score and 11.33\% in Calinski--Harabasz Score over standard K-means.
    
    \item A dual anomaly scoring mechanism that combines attribute-based (40\% weight) and structure-based (60\% weight) scores, providing interpretable anomaly detection at the subgraph level.
    
    \item Comprehensive evaluation across three benchmark datasets (Cora, CiteSeer, PubMed) with comparisons against six baseline methods, ablation studies on weight ratios and threshold parameters, and computational complexity analysis.
\end{enumerate}

The experimental results demonstrate that AnomEn achieves competitive clustering quality compared to state-of-the-art methods while providing the unique capability of subgraph-level anomaly detection. The ablation studies confirm that the framework is robust to hyperparameter variations, with the 0.4/0.6 weight ratio and 15\% threshold providing optimal detection performance.

\subsection{Limitations and Future Work}

While AnomEn demonstrates effectiveness in subgraph anomaly detection, several directions for future work remain. First, the current evaluation relies on synthetic anomaly injection due to the absence of ground-truth anomaly labels in standard citation network datasets. Evaluation on datasets with real-world anomaly annotations (e.g., financial fraud networks, intrusion detection datasets) would provide stronger validation. Second, extending the framework to dynamic graphs where the network structure evolves over time would broaden its applicability. Third, exploring adaptive threshold learning mechanisms instead of fixed percentage-based thresholds could improve detection sensitivity across diverse graph structures. Finally, scaling the CISS algorithm for very large graphs (millions of nodes) through sampling-based approximations is an important direction for practical deployment.
